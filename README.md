...
o get Express installed, now that Node and NPM is installed, we can use it's magical package install solutions. Because we will want Express available as an executable from everywhere within our environment, pass in the -g flag for 'global'.
...
n the last few lines, there are some instructions. The fist one we need to run is the command to install your new app's dependencies. Running npm install inside the dir of your new project folder of donuts will execute the package.json file. If you were to open that up, you would see the following:
